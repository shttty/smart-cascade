...
3. Select the runner/profile from explicit run settings (including `project.profile`), an existing project `.smart-cascade/override.yaml`, then the active/default OMP profile. Record the selection and use it consistently for Root and its installed roles; read existing overrides without rewriting them during startup. Adapter checks and their receipts are not startup prerequisites. After the user's one explicit run-level authorization, send the complete approved queue boundary to Root through the installed `herdr` skill.
