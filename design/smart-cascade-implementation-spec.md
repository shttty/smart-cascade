...
- The host runner owns subagent dispatch, sessions, correlation between a dispatch and its result, communication, status and result delivery, temporary isolation, artifact capture, cleanup, and recovery. The core owns only scheduling, verification of the actual work, decisions, Git integration, and dependency advancement. It does not restate, re-derive, or re-attest what the runner already reports.
...
- Task completion uses strict structured output. The runner reports the retained patch itself; children never author a patch path.
- Acceptance rests on the work, not on paperwork about the work. A result is read as the runner delivers it, and is then verified by applying the retained patch to a clean checkout of the explicit base and inspecting the real bytes, changed paths, and acceptance-target verification. Neither a completion notice nor a self-report is acceptance.
