...
`.smart-cascade/queue.toml` contains static intent and hard boundaries only: stable slice IDs, dependencies, scope, normalized write sets, and named checks. It contains no child topology, runtime status, attempts, sessions, worktrees, candidates, decisions, or parallel flag; the Leader decides child decomposition at runtime.
