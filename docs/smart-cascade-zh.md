...
| 标志 | 效果 |
|---|---|
| `--force-cascade` | 跳过简单路径 — 强制所有任务走完整 Phase 1-4 级联，无论判断者的复杂度评估结果。适用于所有任务都需要经过规划者 + 顾问 + 执行者链路的场景（如安全敏感工作、执行 superpowers 计划）。 |
...
**`--force-cascade` 覆盖：** 如果设置了此标志，完全跳过简单路径 — 无论复杂度评估结果，将所有任务视为中等并分发给规划者。适用于所有任务都必须经过完整规划者 + 顾问 + 执行者链路的场景（如安全敏感工作、执行 superpowers 计划中每个任务都需要质量把关）。

如果是简单任务（且未设置 `--force-cascade`）：**判断者直接执行** — 不分发任何 subagent，不进入级联。
如果是中等/规划任务（或设置了 `--force-cascade`）：**判断者分发规划者 subagent** 并退出，规划者接管后续所有阶段。
...
Agent:
  ...
  model: "{PLANNER_MODEL}"
  prompt: |
    你是一个规划者（PLANNER ONLY）。你的唯一职责是产出计划。
    不要写文件、编辑文件或执行命令。不要实现任何内容。
    实现工作由后续阶段的执行者 agent 专门负责。
    仔细考虑范围、风险和方法。
...
- **执行者 worker 失败（崩溃，非 BLOCKED）** → 用 `haiku`（最低成本模型）重试一次。如果再次失败 → 用 `{PLANNER_MODEL}`（配置的规划者模型作为临时 worker）重试一次，注明：`> *执行者在任务 {id} 上崩溃 — 用 {PLANNER_MODEL} 作为降级 worker 重试。*` 如果仍然失败 → 通知用户。在 Phase 6 摘要中报告所有降级尝试。
