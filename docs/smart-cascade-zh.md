---
name: smart-cascade
description: "分层模型编排，适用于中等到复杂任务。必须通过 /smart-cascade 显式调用 — 永不自动触发。执行者直接处理简单工作。中等/规划任务升级到规划者规划、顾问审查，然后拆分为原子任务分发给并行执行者 worker。Worker 失败时通过内联规划者 advisor agent 升级处理。"
---

# Smart Cascade — 分层模型编排

根据复杂度在执行者 → 规划者 → 顾问 → 执行者 worker 之间路由任务。
...
**解析优先级（从高到低）：**
1. 调用时传入的 CLI 参数（`--advisor=...`）
2. 与 skill 文件同级的 `smart-cascade.json` 配置文件
3. 内置默认值（`opus` / `sonnet` / `haiku`）

**读取配置：** 每次级联开始时，依次检查 CLI 参数、读取同级目录下的 `smart-cascade.json`（如存在）、最后回退到内置默认值，解析三个模型变量 — `{ADVISOR_MODEL}`、`{PLANNER_MODEL}`、`{EXECUTOR_MODEL}`。接受任何有效的 Claude 模型 ID（如 `claude-opus-4-5`、`claude-sonnet-4-5`、`claude-haiku-4-5`）。

## Phase 0：复杂度门控

在执行任何操作之前先评估任务，根据复杂度路由：

| 复杂度 | 信号 | 操作 |
|---|---|---|
| **简单** | 单次问答、单文件、少于 3 步、无需规划 | 直接处理 — 跳过级联 |
| **中等** | 多文件、功能实现、调试、需要规划 | 从 Phase 1 进入级联 |
| **规划** | 架构设计、跨服务、需要任务拆分 | 从 Phase 1 进入级联 |

如果是简单任务：正常响应，不要进入级联。

**模型层级快捷路径：**
- **以 {EXECUTOR_MODEL} 运行：** 分发 {PLANNER_MODEL} subagent 执行 Phase 1，然后正常推进。
- **以 {PLANNER_MODEL} 运行：** 跳过 Phase 1 subagent 分发 — 直接自己规划。规划完成后，自我评估信心并输出 `CONFIDENT:` 或 `UNCERTAIN:` 信号（与 Phase 1 格式相同）。然后使用该信号进入 Phase 2 的 Path A/B 路由。
...
- 每个任务最多三次升级：BLOCKED → 规划者 → 重试 → 规划者 → 重试 → 如果再次 BLOCKED，通知用户。
- 升级始终是内联 agent 调用 — 不依赖外部 skill。
- **规划者规划，不执行。** 规划者的角色是规划、精炼和升级指导。它不得直接执行分配给执行者 worker 的任务 — 即使是部分执行也不行。显式例外（这些是最后手段的降级，不是正常流程）：
